...
def encode(path):
    ...
    np.savetxt(path + "_shape.txt", _shape, fmt = '%d')
    img2 = img.flatten()
    ...
    padded_encoded_text = pad_encoded_text(en_code)
...
def decode(path):
    ...
    _shape = np.loadtxt(path+"_shape.txt", dtype=int)
    huff = np.load(path+"_dict.npy")
    with open(path+"_zip.bin", "rb") as file:
        ...
        en_code = remove_padding(bit_string)
    ...
    np.save("Code_tree.npy", code_tree)
    f = open("Code_tree.bin", "wb")
    f.write(code_tree)
    f.close()
